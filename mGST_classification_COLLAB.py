...
def GST_Pyramid_complete(data, variation, args):
    ...
    for k,(g, labels) in enumerate(data):
        ...
        bar.next()
    bar.finish()
    np.save('/DATA7_DB7/data/gjliu/dataset/'+args.dataset+'/allphi_p_'+args.dataset+'_'+str(args.variation)+'_'+args.sample+'_'+args.gstcoe_norml+'_'+args.fake_signal+'.npy',GSTcoe_all_p)
    GSTcoe_all = GSTcoe_all_p.reshape(np.shape(GSTcoe_all_p)[0],np.shape(GSTcoe_all_p)[1],np.shape(GSTcoe_all_p)[2]*np.shape(GSTcoe_all_p)[3])
    return  GSTcoe_all, label_all #

## 一些比较好的数据记录----------------------------
#batchsize-16\\dataset-COLLAB\\lr=0.1\\step_size=int(100), gamma=0.5, midnum=200---66.5
...
if __name__ == '__main__':
    ...
    parser.add_argument("--dataset", type=str, default='COLLAB', help="name of dataset")
    ...
    os.environ["CUDA_VISIBLE_DEVICES"] = "0,5,6,7,8"
    ...
    if args.waytogetphi == "store" :
        print('load phi.......')
        GSTcoe_all = np.load('/DATA7_DB7/data/gjliu/dataset/COLLAB/allphi_COLLAB.npy')
        print('load label.......')
        label_all = np.load('/DATA7_DB7/data/gjliu/dataset/COLLAB/alllabel_COLLAB.npy')
        print('load complete')
