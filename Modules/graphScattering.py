...
zeroTolerance = 1e-6 # Values below this number are considered zero.
...
class GraphScatteringTransform:
    ...
    def computeTransform_map(self, x):
        # Check the dimension of x: batchSize x numberFeatures x numberNodes
        assert len(x.shape) == 3
        B = x.shape[0] # batchSize
        F = x.shape[1] # numberFeatures
        ...
        Map = np.expand_dims(x, axis=3)  # B x F x N x 1
        ...
        for l in range(1,self.L): # l = 1,2,...,L
            ...
            for j in range(self.J ** (l-1)): # j = 0,...,l-1
                ...
                thisRhoHx = thisRhoHx.transpose(0, 2, 3, 1) # B x F x N x J
                Map = np.concatenate((Map, thisRhoHx), axis = 3)
