...
if __name__ == '__main__':
    data_source = 'map' # 'map' or 'dataset'
    ## 载入graph
    ...
    if data_source == 'map':
        ...
        for i in range(2):
            ...
            Laplacian = laplacian(A_multiscale, laplacian_basic = 'normalized')
            R_Laplacian = Kron_Reduction(Laplacian, node_sampled, node_sampled_c, mode='kron')
            sampled_lenth = np.shape(R_Laplacian)[0]
            A_multiscale = np.multiply(R_Laplacian,(np.eye(sampled_lenth)-np.ones((sampled_lenth,sampled_lenth))))
            A_multiscale[np.isnan(A_multiscale)] = 0
            A_multiscale[np.abs(A_multiscale)<1e-8]=0

            plt.figure()
            draw_graph(A_multiscale, node_attr)
            plt.savefig('/DB/rhome/gjliu/workspace/graph_scattering/test_graph/test'+str(i+1)+'.jpg')


    ###--------------------------------------------------------------------------------
    # 画图试试
    # draw_coords(coords)
    # draw_diffusion_filter_vertex(j=1, adjacancy=A, coords=coords)
    signal_type = "allpass"
    graph_signal = signal_generator(k=0.1, adjacancy=A, gtype=signal_type, coords=coords, D = data)
    plt.savefig('/DB/rhome/gjliu/workspace/graph_scattering/test_graph/'+signal_type+'/graph_signal.jpg')
    ###--------------------------------------------------------------------------------

    lazyT = lazy_diffusion(A)

    ###-----------------------------------------------------------------------------------
    # GST
    GSTmodel = np_GST.DiffusionScattering(3, 3, A)
    node_attr = np.expand_dims(np.expand_dims(graph_signal, axis=0), axis=0)
    co_GST = GSTmodel.computeTransform(node_attr)
    map_GST = GSTmodel.computeTransform_map(node_attr)
    write_csv(co_GST[0,0,:], mode="w+")

    draw_GSTmap(map_GST, coords, J=3, L=3)
    plt.savefig('/DB/rhome/gjliu/workspace/graph_scattering/test_graph/'+signal_type+'/filter_results.jpg')
    ###-----------------------------------------------------------------------------------
    ...
    for j in range(3):
        plt.subplot(2,4,2+j)
        draw_diffusion_filter_spectrum(j+1, adjacancy=A)
        plt.subplot(2,4,6+j)
        draw_diffusion_filter_vertex(j+1, adjacancy=A, coords=coords)
    plt.savefig('/DB/rhome/gjliu/workspace/graph_scattering/test_graph/'+signal_type+'/US_map.jpg')

    k=0
    for trick1 in range(2):
        for trick2 in range(2):
            for trick3 in range(2):
                A = np.power(data['DIST'], -1)
                A[np.isinf(A)] = 0
                plt.figure(dpi=300,figsize=(32,40))##
                for i in range(3):
                    if i == 0:
                        node_sampled, node_sampled_c = downsample(A, method='umin', laplacian_basic = 'unnormalized', trick = trick1)
                    if i == 1:
                        node_sampled, node_sampled_c = downsample(A, method='umin', laplacian_basic = 'unnormalized', trick = trick2)
                    if i == 2:
                        node_sampled, node_sampled_c = downsample(A, method='umin', laplacian_basic = 'unnormalized', trick = trick3)
                    coords_down = coords[node_sampled]
                    coords_down_c = coords[node_sampled_c]
                    plt.subplot(6,4,8*i+1)
                    # draw_two_coords(coords, coords_down_c)

                    draw_coords(coords_down_c)

                    A[node_sampled,:] = 0
                    A[:,node_sampled] = 0
                    
                    A_subgraph = A[np.squeeze(node_sampled_c),:]
                    A_subgraph = A_subgraph[:,np.squeeze(node_sampled_c)]
                    ###-----------------------------------------------------------------------------------
                    # GST
                    GSTmodel = np_GST.DiffusionScattering(3, 3, A_subgraph)
                    node_attr = np.expand_dims(np.expand_dims(graph_signal[node_sampled_c], axis=0), axis=0)
                    co_GST = GSTmodel.computeTransform(node_attr)
                    map_GST = GSTmodel.computeTransform_map(node_attr)
                    write_csv(co_GST[0,0,:], mode="a+")
                    draw_GSTmap(map_GST, coords_down_c, J=3, L=3)
                    plt.savefig('/DB/rhome/gjliu/workspace/graph_scattering/test_graph/'+signal_type+'/US_filtered_map'+str(k)+'.jpg')
                    ###-----------------------------------------------------------------------------------                    
                    plt.figure(dpi=300,figsize=(14,14))
                    for j in range(3):
                        plt.subplot(3,3,3*j+1)
                        draw_diffusion_filter_spectrum(j, adjacancy=A_subgraph)
                        plt.subplot(3,3,3*j+2)
                        draw_diffusion_filter_spectrum_real(j+1, adjacancy=A_subgraph, Map = map_GST, signal = graph_signal[node_sampled_c])
                        plt.subplot(3,3,3*j+3)
                        draw_input_relu_spectrum(j+1, adjacancy=A_subgraph, Map = map_GST, signal = graph_signal[node_sampled_c])
                        # draw_diffusion_filter_vertex(j+1, adjacancy=A_subgraph, coords=coords_down_c)
                    plt.savefig('/DB/rhome/gjliu/workspace/graph_scattering/test_graph/'+signal_type+'/diffusion_filter'+str(k)+'.jpg')
                    k=k+1
                    print('k=',k)
                    # draw_coords(coords)
